...
require_once( dirname(__FILE__).'/InterpretTask.php' );
...
$config->daemonChild = $config->parameters['daemon_interpret_max_child'];
...
// daemon creation
try {
	$daemon = InterpretTask::getInstance();
} catch( Exception $e) {
	echo $e->getMessage()."\n";
	exit( -1 );
}
...
$daemon->setCallback( array('InterpretTask','_exit') );
