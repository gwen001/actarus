...
class RunTask extends Daemon
{
	...
	private function loop()
	{
		...
		{
			$q = "SELECT id FROM arus_entity_task WHERE status='".$this->config->parameters['task']['status']['waiting']."' AND task_id IN (".$this->config->taskPriority.") AND task_id NOT IN (".$this->config->taskIgnore.") ORDER BY id ASC LIMIT 0,".$this->getFreePlace();
		...
		if( $this->getFreePlace() )
		{
			$q = "SELECT id FROM arus_entity_task WHERE status='".$this->config->parameters['task']['status']['waiting']."' AND task_id NOT IN (".$this->config->taskIgnore.") ORDER BY id ASC LIMIT 0,".$this->getFreePlace();
			$result = $this->config->db->query( $q );
			if( !$result ) {
				$this->logger->write( $this->config->db->error().' ('.$this->config->db->errno().') '.$q );
				return;
